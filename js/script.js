(function () {
  ...
  function initContactForm() {
    ...
    if (!form) return;
    var recipient = SITE_DATA.contactEmail || "samathagreenenergy@gmail.com";
    if (location.search.indexOf("sent=1") !== -1) {
      var sentNote = qs("#enquiry-success");
    ...
    form.addEventListener("submit", function (e) {
      ...
      var payload = {
        ...
        _subject: "New enquiry from Samatha Green Energy website",
        _template: "table",
        _captcha: "false",
      };
      if (btn) {
  ...
  function initOfficeMap() {
    var map = SITE_DATA.officeMap;
    if (!map || !map.placeQuery) return;
    var q = map.placeQuery;
    var mapsUrl = "https://www.google.com/maps/search/?api=1&query=" + encodeURIComponent(q);
    var embedSrc = "https://www.google.com/maps?q=" + encodeURIComponent(q) + "&hl=en&z=17&output=embed";
    qsa("a[data-office-map-link]").forEach(function (a) {
      a.setAttribute("href", mapsUrl);
    });
    qsa("iframe[data-office-map-embed]").forEach(function (frame) {
      frame.setAttribute("src", embedSrc);
      frame.setAttribute("title", "Samatha Green Energy Solutions on Google Maps");
    });
  }
